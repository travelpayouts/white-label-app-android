#!/usr/bin/env bash
set -uo pipefail
# Проверяет узкий, заранее известный список наших ключей и учётных данных:
# файлы Firebase с нашими project_id, хранилища подписи по расширениям
# .keystore/.jks/.p12/.bks, файл signing.properties и пять полей в
# config/app_config.json. Это не сканер секретов: файл с другим расширением,
# переименованный properties или чужой ключ в неожиданном месте проверка не
# увидит.
#
# И ещё: проверяется только текущее состояние файлов. История git сюда не
# входит, а партнёр получает репозиторий вместе с ней — если секрет когда-то
# был закоммичен, замена файла его не убирает.
#
#   ./check-no-secrets.sh
#
...
if [ "$FAILED" -eq 0 ]; then
    ...
    echo "Это не гарантия отсутствия секретов — список проверок узкий, см. шапку файла."
else
